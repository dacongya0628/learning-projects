...
import { addTierRow, applyGlobalDrawModeToAllTierRows, applyGlobalRollStopToAllTierRows, bindTierListActions, delay, fillDefaultTiers, mountShell, playRevealAnimation, playRevealBatchAnimation, readTiers, renderResults, setDrawingUiLocked, setError, setPhase, suggestNextTierName, type Elements, type RevealRollOptions } from './ui';
...
/** 各等级剩余名额，用于下拉选项 */
function refreshStageTierOptions(els: Elements, tiers: PrizeTier[], remainingMap: Map<string, number>, preferredId: string | null): void {
...
async function runLottery(els: Elements, signal: AbortSignal): Promise<void> {
    ...
    while (drawn < total) {
        ...
        els.stageHint.textContent = tier.drawMode === 'auto' ? `「${tier.name}」自动揭晓 · 本等级约 ${totalBatchesThisTier} 批 · ${rollHint} · 可随时改选等级` : `「${tier.name}」手动揭晓（除首批外每批点「抽取本批」）· ${rollHint} · 可随时改选等级`;
        ...
        if (lastBatchDrawMode === 'manual' && tier.drawMode === 'auto') {
            ...
            els.stageHint.textContent = tier.drawMode === 'auto' ? `「${tier.name}」自动揭晓 · 本等级约 ${tb} 批 · ${rh} · 可随时改选等级` : `「${tier.name}」手动揭晓（除首批外每批点「抽取本批」）· ${rh} · 可随时改选等级`;
        }
        ...
        try {
            ...
            updateProgress(els, `进度：${drawn} / ${total} · ${tier.name} 第 ${br} 批 · 本批 ${take} 个`);

            const rollOpts = buildRollOpts(tier, els, signal);
            if (nums.length === 1) {
                await playRevealAnimation(els.stageNumber, els.stageParticles, min, max, nums[0], ROLL_MS, signal, rollOpts);
            } else {
                await playRevealBatchAnimation(els.stageNumber, els.stageParticles, min, max, nums, ROLL_MS, signal, rollOpts);
            }
...
async function markAbsentAndRedraw(els: Elements, resultId: string): Promise<void> {
    ...
    try {
        ...
        await playRevealAnimation(els.stageNumber, els.stageParticles, min, max, next, ROLL_MS, ac.signal, rollOpts);
        ...
        els.stageHint.textContent = els.appMain.classList.contains('lottery-complete') ? '可点击「返回抽奖设置」重新配置，或「重置本轮」再抽一次' : (prevHint ?? '');
    }
}
...
function resetStageCopy(els: Elements): void {
    ...
    els.stageHint.textContent = '先选「当前抽取等级」再点「开始抽奖」；手动批点「抽取本批」；滚动手动停点「停止」';
    clearStageRevealDisplay(els);
...
function init(): void {
    ...
    els.btnGoStage.addEventListener('click', () => {
        ...
        scrollIntoViewIfNarrow(els.drawView, {
            behavior: 'smooth',
            block: 'start'
        });
    });
